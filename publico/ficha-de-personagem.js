...
// const url = url_base.replace(/http.*:\d{4}.*usuario\//, '')
...
const url_base = ()=>{
  return url().replace(/http.*:\d{4}.*usuario\//, '')
}

const usuario = ()=>{
  return url().replace(/http.*:\d{4}.*usuario\//, '').split('/')[0]
}
const token = ()=>{
  return url().replace(/http.*:\d{4}.*usuario\//, '').split('/')[1]
}
    ...
    <div class="editar">
      <div class="row">
        ...
        <div class="col">
          ...
          <input type="text" name="ITEM_${numero}_VOLUME" value="L">
        </div>
        <div class="col">
